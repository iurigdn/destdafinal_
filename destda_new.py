...
# Atualização em 20/10/2023: Inclusão do vampo de ST
...
def putOk(table, empresa, path):
    try:
        ...
        try:
            table.loc[table['empresa'] == empresa, 'H'] = 'ok'
...
def run(path):
    ...
    for empresa, dia, comenc, semenc, difativo, difauc,st in zip(empresas, dias, comencs, semencs, difativos, difaucs, sts):
        notify('Usuário clicou em Iniciar')
